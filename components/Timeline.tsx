    ...
    <section className="py-40 relative">
      <div className="max-w-4xl mx-auto px-6">
        <div className="text-center mb-24">
            <h2 className="serif text-5xl text-rose-100 mb-6">Our Narrative</h2>
            <div className="h-1 w-20 bg-rose-600 mx-auto rounded-full neon-border" />
        </div>
            ...
            {events.map((ev, i) => (
              ...
              >
                <div className={`flex-1 w-full text-center ${i % 2 === 0 ? 'md:text-right' : 'md:text-left'}`}>
                  <div className="glass-card p-8 rounded-3xl neon-border hover:shadow-[0_0_30px_rgba(225,29,72,0.2)] transition-shadow group">
                    <span className="text-[10px] uppercase tracking-[0.4em] text-rose-400 font-bold mb-2 block">{ev.date}</span>
                    <h3 className="serif italic text-3xl text-rose-100 mb-4 group-hover:text-rose-400 transition-colors">{ev.title}</h3>
                    <p className="text-rose-100/60 font-light text-sm leading-relaxed">{ev.desc}</p>
                  </div>
                </div>
                
                <div className="absolute left-[20px] md:left-1/2 md:-translate-x-1/2 z-10 w-10 h-10 rounded-full glass-card flex items-center justify-center neon-border bg-[#0c0206]">
